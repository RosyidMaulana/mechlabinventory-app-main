...
class PinjamPengembalianController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        for ($i = 0; $i < $totalBarangDipinjam; $i++) {
            ...
            $validated['code_item'] = $item->code_item;
            ...
            Item::where('code_item', $validated['code_item'])->update([
                'borrowed' => $stok_dipinjam + $request->input('listBrgPinjam')[$i]["qty"],
                'stock' => $stok_tersedia - $request->input('listBrgPinjam')[$i]["qty"],
            ]);
    ...
    public function update(Request $request, PostPinjam $postPinjam, $id)
    {
        ...
        foreach ($detail as $dt) {
            ...
            DetailLoan::where('loan_id', $idDetail)->update(['qty' => 0]);
        }
